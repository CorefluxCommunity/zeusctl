...
import (
	...
	"github.com/CorefluxCommunity/zeusctl/pkg/yubikeypgp"
...
func decryptUnsealKey(yks *yubikeyscard.YubiKeys, cipherTxtB64 string) (unsealKey string, err error) {
	...
	for retries > 0 {
		...
		if err != nil {
			switch {
			case retries == 0:
				utils.PrintFatal("PIN bank locked, no retries remaining", 1)
			case retries < 0:
				return "", err
			default:
		...
		utils.PrintInfo(fmt.Sprintf("decrypted unseal key with key ID %X found on YubiKey %x", md.DecryptedWith, serial))
